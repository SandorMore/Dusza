...
const LeaderCardCreator: React.FC<Props> = ({ worldCards, onCardCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Vezérkártya sikeresen elkészítve!')
      ...
      if (error instanceof Error) {
        ...
        setMessage('Ismeretlen hiba történt a kártya létrehozásakor.')
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-semibold mb-4">Vezérkártya létrehozása: </h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">Alapkártya kiválasztása</label>
          <select
            required
          ...
          >
            <option value="">Fejlesztendő kártya kiválasztása</option>
        ...
        {selectedCard && (
          <div className="bg-gray-50 p-3 rounded-md">
            <p className="text-sm text-gray-600">Kiválasztott kártya:</p>
            <p className="font-medium">{selectedCard.name}</p>
            <p className="text-sm">Sebzés: {selectedCard.damage} | Életerő: {selectedCard.health}</p>
          </div>
        )}

        <div>
          <label className="block text-sm font-medium text-gray-700">Vezérkártya neve</label>
          <input
            type="text"
            required
            ...
            className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-violet-500 focus:border-violet-500"
            placeholder="Vezérkártya neve"
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700">Fejlesztés típusa</label>
          <div className="mt-2 space-y-2">
            <label className="inline-flex items-center">
              ...
              />
              <span className="ml-2">Dupla sebzés (2×)</span>
            </label>
            <label className="inline-flex items-center ml-6">
              <input
                ...
                className="text-violet-500 focus:ring-violet-500"
              />
              <span className="ml-2">Dupla életerő (2×)</span>
            </label>
          </div>
        </div>
        ...
        {selectedCard && (
          <div className="bg-blue-50 p-3 rounded-md">
            ...
            <p className="text-sm">
              ...
              Sebzés: {formData.boostType === 'damage' ? selectedCard.damage * 2 : selectedCard.damage} - 
              Életerő: {formData.boostType === 'health' ? selectedCard.health * 2 : selectedCard.health}
            </p>
          </div>
        )}
        ...
        >
          {loading ? 'Vezérkártya elkészítése...' : 'Vezérkártya létrehozása'}
        </button>
